...
public class MeshAsset
{
    ...
    public void Upload(MeshUploadData meshUploadData)
    {
            ...
            switch (type)
            {
                ...
                case VertexAttributeType.Color:
                {
                    if (dimension is 4)
                    {
                        ...
                        {
                            vertexActions.Add(() => colorList.Add(new Color((float)vertReader.ReadHalf(), (float)vertReader.ReadHalf(), (float)vertReader.ReadHalf(), (float)vertReader.ReadHalf())));
                            used = 2 * 4; //sizeof(Half) * 4
                        }
                        else if (format is VertexAttributeFormat.UNorm8)
                        {
        ...
        for (var i = 0; i < meshBuffer.SubmeshCount; i++)
        {

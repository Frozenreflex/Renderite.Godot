...
public partial class RendererManager : Node
{
    ...
    public override void _Ready()
    {
        ...
        var shmPrefix = new string(Enumerable.Range(0, 16)
            ...
            .ToArray());
        var bootstrapperIn = new QueueFactory().CreateSubscriber(new QueueOptions(shmPrefix + ".bootstrapper_in", 8192L, deleteOnDispose: true));
        var bootstrapperOut = new QueueFactory().CreatePublisher(new QueueOptions(shmPrefix + ".bootstrapper_out", 8192L, deleteOnDispose: true));
        ...
        var queueArgs = Encoding.UTF8.GetString(bootstrapperIn.Dequeue(CancellationToken.None).Span).Split(' ');
        var queueName = queueArgs[1];
        var queueCapacity = long.Parse(queueArgs[3]);

        bootstrapperOut.TryEnqueue(Encoding.UTF8.GetBytes("RENDERITE_STARTED:" + System.Environment.ProcessId));
